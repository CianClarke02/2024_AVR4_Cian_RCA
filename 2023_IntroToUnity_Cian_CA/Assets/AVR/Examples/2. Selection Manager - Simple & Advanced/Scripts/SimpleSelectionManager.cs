using UnityEngine;

namespace AVR
{
    public class SimpleSelectionManager : MonoBehaviour
    {
        [SerializeField]
        private string selectableTag = "Selectable";

        [SerializeField]
        [Tooltip("Played on first selection")]
        private AudioClip audioClip;

        [SerializeField]
        [Range(0, 1)]
        private float audioVolume = 1;

        [SerializeField]
        private GameObject highlightTargetObject;

        //local vars
        private Ray ray;

        private Camera mainCamera;
        private HighlightBehaviour hightlightBehavior;
        private Transform previousSelection;

        private void Awake()
        {
            selectableTag = selectableTag.Trim();
            mainCamera = Camera.main;

            hightlightBehavior
                = highlightTargetObject.GetComponent<HighlightBehaviour>();

            highlightTargetObject.SetActive(false);
        }

        // Update is called once per frame
        private void Update()
        {
            //get a line(ray) from camera to mouse position into world
            ray = mainCamera.ScreenPointToRay(Input.mousePosition);

            //prepare an object to store hit results
            RaycastHit hitInfo = new RaycastHit();

            //work out what was hit/moused over
            if (Physics.Raycast(ray, out hitInfo))
            {
                //get transform of hit thing
                var currentSelection = hitInfo.transform;

                //did we hit something interesting?
                if (currentSelection.CompareTag(selectableTag))
                {
                    if (currentSelection != previousSelection)
                        AudioSource.PlayClipAtPoint(audioClip,
                            currentSelection.position, audioVolume);

                    hightlightBehavior.SetPosition(currentSelection.position);

                    highlightTargetObject.SetActive(true);
                }

                previousSelection = currentSelection;
            }
        }
    }

    /*
    public class SimpleSelectionManager : MonoBehaviour
    {
        [SerializeField]
        private string selectableTag = "Selectable";

        [SerializeField]
        [ColorUsage(true, true)]
        private Color currentSelectedColor;

        private Camera mainCamera;

        //local vars
        private Ray ray;

        private Transform previousSelection;
        private Color previousColor;

        private void Awake()
        {
            selectableTag = selectableTag.Trim();
            mainCamera = Camera.main;
        }

        // Update is called once per frame
        private void Update()
        {
            //get a line(ray) from camera to mouse position into world
            ray = mainCamera.ScreenPointToRay(Input.mousePosition);

            //prepare an object to store hit results
            RaycastHit hitInfo = new RaycastHit();

            //work out what was hit/moused over
            if (Physics.Raycast(ray, out hitInfo))
            {
                //get transform of hit thing
                var currentSelection = hitInfo.transform;

                //did we hit something interesting?
                if (currentSelection.CompareTag(selectableTag))
                {
                    //you've selected something different
                    if (currentSelection != previousSelection)
                    {
                        Material mat = currentSelection.gameObject
                            .GetComponent<MeshRenderer>().material;
                        //store old
                        previousColor = mat.color;

                        //set to new
                        mat.color = currentSelectedColor;
                    }

                    previousSelection = currentSelection;
                }
            }
            //nothing selected
            else
            {
                if (previousSelection != null)
                {
                    Material prevMat = previousSelection.gameObject
                               .GetComponent<MeshRenderer>().material;
                    prevMat.color = previousColor;
                    previousSelection = null;
                }
            }
        }
    }
    /*

    /*
      public class SimpleSelectionManager : MonoBehaviour
    {
        [SerializeField]
        private string selectableTag = "Selectable";

        private Camera mainCamera;

        //local vars
        private Ray ray;

        private void Awake()
        {
            selectableTag = selectableTag.Trim();
            mainCamera = Camera.main;
        }

        // Update is called once per frame
        private void Update()
        {
            //get a line(ray) from camera to mouse position into world
            ray = mainCamera.ScreenPointToRay(Input.mousePosition);

            //prepare an object to store hit results
            RaycastHit hitInfo = new RaycastHit();

            //work out what was hit/moused over
            if (Physics.Raycast(ray, out hitInfo))
            {
                //get transform of hit thing
                var currentSelection = hitInfo.transform;

                //did we hit something interesting?
                if (currentSelection.CompareTag(selectableTag))
                {
                    //change the moused over thing
                    Debug.Log(currentSelection.position.ToString());
                }
            }
        }
    }

     */
}